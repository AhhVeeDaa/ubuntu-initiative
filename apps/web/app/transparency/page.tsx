...
export default async function TransparencyPage() {
  ...
  const approvalRate = auditLogs ? 
    Math.round((auditLogs.filter(l => l.status === 'approved').length / auditLogs.length) * 100) : 
    0;
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-grow pt-24 pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* Header */}
          <div className="mb-12">
            <h1 className="text-4xl font-bold text-white mb-4">Transparency Dashboard</h1>
            <p className="text-gray-400 text-lg max-w-3xl">
              Real-time operational data for the Ubuntu Initiative. All metrics are automatically 
              aggregated from institutional monitoring agents and human oversight processes.
            </p>
            <div className="mt-4 flex items-center gap-2 text-sm text-gray-500">
              <Clock className="h-4 w-4" />
              <span>Data as of {new Date(lastUpdate).toLocaleString('en-US', {
                year: 'numeric',
                month: 'long',
                day: 'numeric',
                hour: '2-digit',
                  ...
                  <div key={index} className="flex items-start gap-4 pb-4 border-b border-white/10 last:border-0">
                    ...
                    <div className="flex-grow">
                      ...
                      <p className="text-gray-500 text-xs mt-2">
                        {new Date(milestone.timestamp).toLocaleDateString('en-US', {
                          year: 'numeric',
                          month: 'long',
                          day: 'numeric'
                        })}
